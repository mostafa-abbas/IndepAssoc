...
**Does an exposure–outcome association survive being tested five independent ways?**
...
independent methods and shows whether they agree. When they agree, the finding
is robust; when they do not, the signal needs a closer look.

![Conceptual overview: one association in the data, tested five independent ways (regression, matching, stratification, IPTW, AIPW), then a check of whether the methods agree](man/figures/indepassoc-overview.png)
...
- **AIPW (augmented IPTW)** — combines the IPTW reweighting with a regression
  model; a hybrid that is often more stable than either approach alone.

## What it is — and what it is not

**What it is:** a pipeline that tests whether an exposure is independently
associated with an outcome — adjusting for confounding factors via matching,
