...
\<\<\<\<\<\<\< HEAD IndepAssoc answers one question: is a risk factor
(an exposure) genuinely associated with an outcome after other relevant
factors are accounted for — or is the apparent link a statistical
artifact? Rather than trusting a single statistical method, the package
tests the same association through five methods that each rely on
different modeling assumptions, and reports whether they agree. When
they agree, the finding is robust; when they do not, the signal warrants
a closer look before it is written up.
...
accounted for — or is the apparent link a statistical artifact of
whichever method happened to be used? Rather than trusting a single
model, the package runs the same association through five methods that
each rely on different assumptions, and reports plainly whether they
agree. Agreement is a good sign. Disagreement is a prompt to look closer
before writing anything up. \>\>\>\>\>\>\> b0464e1 (Update IndepAssoc to
version 0.6.5 with refined vignettes, pipeline updates, and
documentation)
...
\<\<\<\<\<\<\< HEAD Alternatively, install from source with the package
checked out locally: ======= Running
[`run_pipeline()`](https://mostafa-abbas.github.io/IndepAssoc/reference/run_pipeline.md)
on the RHC cohort below (n = 5,735, 50 confounders) — all five methods
agree RHC is associated with increased 30-day mortality:
...
agree](reference/figures/indepassoc-overview.png) \>\>\>\>\>\>\> b0464e1
(Update IndepAssoc to version 0.6.5 with refined vignettes, pipeline
...
independently associated with an outcome — adjusting for confounding via
five distinct methods — and produces publication-ready tables and effect
estimates with minimal code.

\<\<\<\<\<\<\< HEAD **What it is not:** proof of causation. The package
reports confounder-adjusted associations; it does not establish causal
effects. IPTW and AIPW are causal- inference estimators, but their
output represents a causal effect only if the identifiability
assumptions hold: no unmeasured confounding, positivity (every covariate
pattern has a nonzero chance of both exposure levels), and — for IPTW in
particular — a correctly specified propensity model. An association —
however robust across methods — is not a causal effect, and the package
...
spot in the confounder list. ======= **What it is not:** proof of
causation. IPTW and AIPW are causal-inference estimators, but their
output is a causal effect only if the identifiability assumptions hold —
no unmeasured confounding, positivity, and for IPTW, a correctly
specified propensity model. An association, however robust across
methods, is not a causal effect, and the package does not claim
otherwise.

**What “all five agree” does and doesn’t tell you.** Agreement is
evidence against *estimator-specific* bias — a finding that survives
five different modeling assumptions is unlikely to be an artifact of how
any one was specified. It is not evidence against *unmeasured
confounding*: all five methods adjust for the same covariates, so a
confounder missing from that list would bias all five the same way, and
look identical to a real effect.
...
`causaldata` is additionally needed to build the `causal-benchmarks`
vignette (it supplies the NHEFS data); every other dependency, including
`MatchIt`, installs automatically with the package, and the other three
vignettes need nothing beyond that. \>\>\>\>\>\>\> b0464e1 (Update
IndepAssoc to version 0.6.5 with refined vignettes, pipeline updates,
and documentation)
...
\<\<\<\<\<\<\< HEAD `example_cohort` is a fully simulated dataset
shipped with the package (see [Datasets](#datasets)) — it carries no
patient information of any kind, and exists purely so the pipeline can
be run end-to-end on the first try.
...
it exists purely so the pipeline runs end-to-end on the first try.
[`format_comparison()`](https://mostafa-abbas.github.io/IndepAssoc/reference/format_comparison.md)
rounds `res$comparison` into the table below: \>\>\>\>\>\>\> b0464e1
(Update IndepAssoc to version 0.6.5 with refined vignettes, pipeline
updates, and documentation)
...
\<\<\<\<\<\<\< HEAD The five methods do not all target the same
estimand, and point estimates are not expected to be numerically
identical even when every model is correctly specified:
...
the five effect estimates: ======= The five methods target related but
distinct estimands, so their point estimates are not expected to match
exactly even under correct specification:
...
outcomes the OR is non-collapsible — not numerically equal to a marginal
ATE. Agreement across the five is meaningful robustness evidence, but
they are related quantities, not five copies of one number. Full
derivation of each method’s variance and pooling rule is in the
[technical
vignette](https://mostafa-abbas.github.io/IndepAssoc/articles/rhc-validation.html).

**Diagnostics matter more than the p-value alone.** The IPTW row above
sits right at the significance threshold, and IPTW weights aren’t
trimmed by default — light trimming (`trim = 0.01`) shifts it from p =
0.059 to p = 0.025 on this same data. That’s exactly why the diagnostics
below exist: a borderline weighted estimate is a prompt to check the
weight distribution, not to report it as-is.

**Data requirements:** the exposure, every covariate, and the outcome
must be complete.
[`run_pipeline()`](https://mostafa-abbas.github.io/IndepAssoc/reference/run_pipeline.md)
and
[`fit_outcome()`](https://mostafa-abbas.github.io/IndepAssoc/reference/fit_outcome.md)
error immediately if any required column has missing values, naming the
column and the count — neither function drops rows or imputes silently.
Handle missing data first (complete-case, single imputation, or multiple
imputation with the full pipeline re-run and pooled via Rubin’s rules)
...
Every
[`run_pipeline()`](https://mostafa-abbas.github.io/IndepAssoc/reference/run_pipeline.md)
call automatically checks propensity-score positivity and covariate
balance — worth reviewing before trusting any estimate above:
\>\>\>\>\>\>\> b0464e1 (Update IndepAssoc to version 0.6.5 with refined
vignettes, pipeline updates, and documentation)
...
`summary(res)` reports it plainly: ======= plot(res) \# ASMD Love plot,
...
\<\<\<\<\<\<\< HEAD The post-match ASMD on the propensity-score distance
itself is 0.157, above the default 0.1 threshold, even though all four
individual covariates balance well (0.006–0.076). That doesn’t
invalidate the other four methods’ estimates, but it’s a direct, honest
signal that this particular match deserves scrutiny — a tighter caliper
or an expanded propensity model, and the matching-based estimate
reported alongside, not instead of, the other four. This is what the
diagnostics are for, and the package surfaces it without being asked.
...
======= The post-match ASMD on the propensity-score distance is 0.157,
above the 0.1 threshold, even though all four individual covariates
balance well (0.006–0.076). That doesn’t invalidate the other four
methods — it’s a direct signal that this particular match deserves a
tighter caliper or an expanded propensity model, and that the matching
estimate should be read alongside the other four rather than in place of
them. This is what the diagnostics are for, surfaced without being
asked.

## A real-world validation

The same pipeline was run on the Right Heart Catheterization (RHC)
cohort from the SUPPORT study (Connors et al., *JAMA*, 1996) — a
long-standing, publicly available benchmark in the causal-inference
literature, also distributed via the `causaldata` R package and the
Vanderbilt Department of Biostatistics. No hospital, health-system, or
proprietary patient-level data was used at any stage of development or
testing — every dataset behind these results, including the two further
benchmarks below, is public.
...
reproduces an established finding on real clinical data, not only
simulated cohorts. Full analysis — balance checks, matching diagnostics,
sensitivity checks — is in the [rhc-validation
vignette](https://mostafa-abbas.github.io/IndepAssoc/articles/rhc-validation.html).
\>\>\>\>\>\>\> b0464e1 (Update IndepAssoc to version 0.6.5 with refined
vignettes, pipeline updates, and documentation)
...
[causal-benchmarks
vignette](https://mostafa-abbas.github.io/IndepAssoc/articles/causal-benchmarks.html).
On the NHEFS smoking-cessation cohort (Hernán & Robins), all five
methods converge on both outcomes tested — the strong form of
validation. On the Lalonde job-training dataset (Dehejia & Wahba), only
matching recovers the known experimental benchmark closely; IPTW and
AIPW diverge from it. That’s expected, not a failure: Lalonde’s
non-experimental comparison group is a known, textbook case of poor
covariate overlap between treated and control units, and matching — not
IPTW or AIPW — is the estimator this specific benchmark is understood to
favor. The methods disagreeing exactly where overlap theory predicts
they should is, if anything, a more convincing result than agreement
everywhere would be.

\<\<\<\<\<\<\< HEAD IndepAssoc is the packaged form of the analysis
pipeline used in two published retrospective cohort studies from the
same author group: *[Female sex is associated with short-term mortality
in coronary artery bypass grafting patients: A propensity-matched
...
identical results. ======= \## Package testing & CI
...
checked against `example_cohort` (simulated, known ground truth) and
three public benchmarks with independently known results: `rhc_sample`,
NHEFS, and Lalonde (above).
...
- `rhc_sample` — the public SUPPORT RHC cohort (5,735 rows, 50
  confounders), the basis of the `rhc-validation` vignette. A
  confounder-adjusted association benchmark, not proof of causality (see
  [`?rhc_sample`](https://mostafa-abbas.github.io/IndepAssoc/reference/rhc_sample.md)).

Both are lazy-loaded via [`data()`](https://rdrr.io/r/utils/data.html).
`prepare_rhc_data(path)` is also exported for anyone working with a
fresh raw RHC CSV rather than the bundled `rhc_sample` — it resolves the
source file’s known quirks (a stray `write.csv` row-names column, `"NA"`
stored as literal text, and a text-coded exposure/outcome) into the
typed, numeric-encoded data frame the pipeline expects.

## Reproducibility

Matching draws on the random number generator — set a seed explicitly
(`run_pipeline(..., seed = N)` or `match_cohort(..., seed = N)`) and
reuse it to reproduce results exactly.

## Published Research

IndepAssoc packages the analysis pipeline behind two published
retrospective cohort studies: *[Female sex is associated with short-term
mortality in coronary artery bypass grafting
patients](https://doi.org/10.1016/j.heliyon.2025.e41723)* (Heliyon,
2025) and *[Atrial appendage closure is associated with increased risk
for postoperative atrial
fibrillation](https://doi.org/10.1186/s13019-024-03119-6)* (J.
Cardiothoracic Surgery, 2024) — both built on the same propensity-score
...
- **Usage questions:** [clinician
  ...
  or [benchmark
  validation](https://mostafa-abbas.github.io/IndepAssoc/articles/causal-benchmarks.html)
  (RHC, NHEFS, Lalonde)
